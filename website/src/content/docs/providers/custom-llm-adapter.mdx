...
Receives the full `Message[]` array from a thread (with `id`, `threadId`, `role`, `content`, and `createdAt` fields). Returns an array of extracted facts, each with:

- `content` -- a brief factual statement
- `source` -- either `"confirmed"` (user stated it directly) or `"inferred"` (derived from context)

Vitamem calls this during the dormant transition pipeline to identify facts worth remembering. The implementation typically formats the messages into a prompt and parses the LLM's JSON response.
