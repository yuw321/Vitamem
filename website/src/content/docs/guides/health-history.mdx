...
:::note
Supersede is automatic when `supersedeThreshold` is configured (default: 0.75). Facts with cosine similarity between the supersede threshold and the deduplication threshold (default: 0.92) update the existing memory rather than creating a duplicate. If your app needs to track that a user *stopped* a medication, add explicit extraction guidance for that in your prompt.
:::
