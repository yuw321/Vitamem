...
:::tip[Why enable reflection for health data?]
Health companions deal with medications, dosages, and conditions where accuracy is critical. The [reflection pass](/concepts/reflection/) catches contradictions (e.g., conflicting medication dosages) and enriches vague facts (e.g., "takes pills" becomes "takes metformin 500mg twice daily") before they enter the memory store. The extra LLM call per session is a small cost for significantly higher data quality.
:::
