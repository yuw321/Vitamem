...
<Warning>
Do not use `EphemeralAdapter` in production. All data is lost on process restart. For durable storage, use `SupabaseAdapter` or a [custom adapter](/docs/guides/custom-storage).
</Warning>
...
For building a custom adapter, see the [custom storage guide](/docs/guides/custom-storage).
