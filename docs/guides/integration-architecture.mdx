...
<Warning>
`sweepThreads()` requires a storage adapter that implements the optional `getThreadsByState()` method. Both `EphemeralAdapter` and `SupabaseAdapter` support this. If you build a custom adapter, see the [custom storage guide](/docs/guides/custom-storage).
</Warning>
...
See the [auto-retrieve concept doc](/docs/concepts/auto-retrieve) for details.
