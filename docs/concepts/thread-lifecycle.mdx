...
2. **Cooling -> Dormant**: Finds all `cooling` threads where `coolingStartedAt` is older than `coolingTimeoutMs` and transitions them to `dormant`. The [embedding pipeline](/docs/api-reference/embedding-pipeline) runs for each, extracting and saving memories.
